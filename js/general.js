...
$('#info').on('show.bs.modal', function (event) {
	  ...
	  document.getElementById('cartaresponsiva').setAttribute('href', 'controlers/reader.php?file_name=' + recipient11.trim() +'.jpg')
	  document.getElementById('launchPDF').setAttribute('href', 'controlers/reader.php?file_name=' + recipient11.trim() +'.pdf')
	  document.getElementById('launchXML').setAttribute('href', 'controlers/reader.php?file_name=' + recipient11.trim() +'.xml')
	  document.getElementById('computercode').innerHTML = recipient8;
	});

$('#updRegistry').on('show.bs.modal', function (event) {
	  var button = $(event.relatedTarget) // Button that triggered the modal
	  var recipient0 = button.data('empcode')
	  var recipient1 = button.data('empphone')
	  var recipient2 = button.data('empname')
	  ...
	  modal.find('.modal-body #txt_rCode').val(recipient0)
	  modal.find('.modal-body #txt_rNames').val(recipient2)
	  modal.find('.modal-body #txt_rMail').val(recipient5)
	  modal.find('.modal-body #txt_rPosition').val(recipient4)
	  modal.find('.modal-body #txt_rBranch').val(recipient6)
	  modal.find('.modal-body #txt_rWS').val(recipient7)
	  modal.find('.modal-body #txt_rDate').val(recipient19)
	  ...
	  // document.getElementById('txt_rCode').innerHTML = recipient8;
	});
